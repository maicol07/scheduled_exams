...
if ($listinfo["img"] == null) {
    ...
    <a id="fab-link" class="btn-floating btn-large waves-effect waves-light red tooltipped" onclick="edit_list_mode()"  data-position="left" data-tooltip="' . _("Modifica") . '">
            ...
            <div class="col s12 m7" id="maincontainer">
                ...
                if ($listinfo["contenuto"] != "" OR $listinfo["contenuto"] != null) {
                    ...
                    <tr>
                        ...
                        <th>' . _("Studente") . '</th>
                        <th>' . _("Giornata") . '</th>
                        <th><span class="tooltipped" data-position="top" data-tooltip="' . _("Giustificazione") . '" style="border-bottom: 1px dotted gray;">G</span></th>
                    </tr>
                </thead><tbody>';
                    foreach ($contenuto as $name => $value) {
                        ...
                        if ($a["giustificato"] == "S") {
                            if ($_SESSION["username"] == $name) {
                                $g = '<label><input type="checkbox" checked="checked" id="giustificazione-' . $name . '" onclick="giustificazione(\'' . $name . '\')"/><span class="tooltipped" data-position="right" data-tooltip="' . _("Sei giustificato. Vuoi revocare la giustificazione?") . '" id="giustificazione-span-' . $name . '"></span></label>';
                            } else {
                                $g = '<i class="fal fa-check tooltipped" data-position="right" data-tooltip="' . _("Giustificato") . '"></i>';
                            }
                        } else {
                            if ($_SESSION["username"] == $name) {
                                $g = '<label><input type="checkbox" id="giustificazione-' . $name . '" onclick="giustificazione(\'' . $name . '\')"/><span class="tooltipped" data-position="right" data-tooltip="' . _("Non sei giustificato. Vuoi giustificarti? Giustificazioni rimanenti: ") . $userinfo["giustificazioni_rim"] . '" id="giustificazione-span-' . $name . '"></span></label>';
                            } else {
                                $g = '';
                            }
                        }
                    ...
                    if (in_array($_SESSION["username"], $admins)) {
                        echo '<h6 class="center-align">' . _("Sembra che questa lista sia stata appena creata. Seleziona un' azione:") . '</h6>
<br>
            ...
            <div class="col s12 m5">
                <div class="card hoverable">
                    <?php if ($listinfo["img"] != null) {
                        ...
                        <a id="fab-link" class="btn-floating halfway-fab waves-effect waves-light red tooltipped" onclick="edit_list_mode()"  data-position="left" data-tooltip="' . _("Modifica") . '"><i id="fab-icon" class="material-icons">mode_edit</i></a>
                    </div>
                   ...
                   style="cursor: pointer;" class="btn-flat waves-effect red-text">' . _("Elimina lista") . '</a></div>';
                    }
                    ?>
                </div>
            </div>
